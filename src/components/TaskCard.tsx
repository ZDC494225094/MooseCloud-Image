...
export default function TaskCard({
  ...
  useEffect(() => {
    ...
    if (task.outputImages?.[0]) {
      ...
      const cached = getCachedImage(imageId, 'preview')
      const cachedMeta = getCachedImageMetadata(imageId)
      if (cached) {
        setThumbSrc(cached)
      }
      if (cachedMeta?.width && cachedMeta?.height) {
        setCoverRatio(formatImageRatio(cachedMeta.width, cachedMeta.height))
        setCoverSize(`${cachedMeta.width} x ${cachedMeta.height}`)
      }
      ensureImagePreviewSrc(imageId).then((url) => {
        if (url) setThumbSrc(url)
      })
      ensureImageMetadata(imageId).then((meta) => {
        if (meta?.width && meta?.height) {
          setCoverRatio(formatImageRatio(meta.width, meta.height))
          setCoverSize(`${meta.width} x ${meta.height}`)
        }
      })
    }
  }, [task.outputImages])
  ...
  useEffect(() => {
    if (!thumbSrc || (coverRatio && coverSize)) return
    ...
    image.onload = () => {
      if (!cancelled && image.naturalWidth > 0 && image.naturalHeight > 0) {
        ...
        setCoverSize(`${image.naturalWidth} x ${image.naturalHeight}`)
      }
    }
    image.src = thumbSrc
  ...
  const completedCount = Math.max(task.outputImages.length, task.completedCount ?? 0)
  ...
  const hasPreview = Boolean(thumbSrc)
      ...
      >
        ...
        <div className="flex h-40">
          <div className="w-40 min-w-[10rem] h-full bg-gray-100 dark:bg-black/20 relative flex items-center justify-center overflow-hidden flex-shrink-0">
            {task.status === 'running' && !hasPreview && (
            ...
            {(task.status === 'done' || task.status === 'running') && thumbSrc && (
              <>
                <img
                  src={thumbSrc}
          ...
          <div className="flex-1 p-3 flex flex-col min-w-0">
            ...
            <div className="mt-auto flex flex-col gap-1.5">
              ...
              <div className="flex gap-1 justify-end flex-shrink-0" onClick={(e) => e.stopPropagation()}>
                {task.status === 'error' && (
                  <button
